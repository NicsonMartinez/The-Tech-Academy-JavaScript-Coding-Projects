...
window.onload = function() {watch()}; //Note: call a funtion and that function is watch(), on page load.
function watch() {
...
function rollForTurn() {
	...
	var ranNum = '';
	var minimum = 1;
	var maximum = 11; //Note: because arrays start counting from 0 
    var first = "";
    var txt1 = "";
	for (var i = 0; i < 2; i++) {
		// random whole number between 1 and 10
		ranNum = Math.floor(Math.random()*(maximum - minimum) + minimum); //Note: Math.floor() rounds a number to the nearest integer
		xArray.push(ranNum);
	}
    ...
    return first; //Note: the function that called/started this function (which is startGame) asked for a return 
}
...
function startGame() {
    ...
    if (activePlayer == "") { // if it was a tie, then reroll
    	activePlayer = rollForTurn(); //Note: it was acting buggy when he tries too re-roll 
    }
...
function btnDisabled(btn) {
	...
	btn.disabled = true; //Note: button wont do anything when clicked
}
...
function stopGame() {
	...
	document.getElementById('boardState').innerHTML = ""; //Note: users cant see this on front end, its more for developers
}
...
function saveSettings() {
	var p1Index = document.getElementById("player1").selectedIndex; //Note: the selected "X" or "O"
    var p1Selected = document.getElementById("player1").options;	//Note: the options "X" & "O"
	...
	if (p1Selected[p1Index].text == p2Selected[p2Index].text) {
		...
		document.getElementById('p1Display').innerHTML=p1Selected[p1Index].text; //Note: inside class="console_left"
...
function getAvatars() {
	var p1Avatar = document.getElementById("p1Display").innerHTML; //Note: we are getting the text inside the tag ("X" or "O")
    var p2Avatar = document.getElementById("p2Display").innerHTML; //Note: we are getting the text inside the tag ("X" or "O")
...
function determineAvatar() {
	...
	if (active == "Player 1") { // check which player is active and their corresponding avatar
		var paintAvatar = p1Avatar; //Note: "X" or "O" depending on what happened earlier
	} else if (active == "Player 2") {
		var paintAvatar = p2Avatar; //Note: "X" or "O" depending on what happened earlier
	}
...
function square1Animate() {
	var activePlayer = document.getElementById('showPlayer').innerHTML; //Note: if it's player one or player two
